...
# use --ruby flag will enable ruby cache
# The cache configuration will enable the command line
# options

# FalseSharing: options moved to Options.py file
...
def create_system(options, full_system, system, dma_ports, bootmem,
    ...
    if buildEnv['PROTOCOL'] != 'FS_MESI':
        fatal("This script requires the FS_MESI protocol to be built.")
    ...
    if options.global_act_size:
        global_act_size = options.global_act_size
        assoc_act = options.assoc_act
    ...
    for i in range(options.num_l2caches):
        ...
        global_act = FSGlobalACT(global_act_size=global_act_size,
                                 tracking_width=tracking_width,
                                 assoc_act=assoc_act,
                                 offset_bit_index=block_size_bits,
