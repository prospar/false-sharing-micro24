...
BENCH_OPTIONS=("" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "-p4 -n8192 -b16 -s" "-p4 -n8192 -b16 -s" "" "" "" "" "" "")
ALL_PROTOCOLS=("MESI_Two_Level" "MESI_Two_Level_Extended" "FS_MESI_DETECTION" "FS_MESI" "FS_MESI_Blocking" "FS_MESI_DETECTION_Blocking" "MESI_Nonblocking")
...
BUILD_CMD="scons build/X86_$INPUT_PROTOCOL/gem5.opt -j5 --default=X86 PROTOCOL=$INPUT_PROTOCOL SLICC_HTML=True"
eval "$BUILD_CMD"
...
#echo $SIZE_OWN $RESET_TICK 
CMD="build/X86_$INPUT_PROTOCOL/gem5.opt --outdir=$OUTDIR configs/example/se.py --ruby --cpu-type='$CPU_TYPE' --mem-size='$PRIMARY_MEM' --num-cpus=$NUM_CPUS --cacheline_size=$CACHELINE_SIZE --l1d_assoc=$L1D_ASSOC --l1d_size=$L1D_SIZE --l1i_assoc=$L1I_ASSOC --l1i_size=$L1I_SIZE --l2_assoc=$L2_ASSOC --l2_size=$L2_SIZE --tracking_width=$GRANULARITY --inv_threshold=$INV_TH --fetch_threshold=$FETCH_TH --global_act_size=$ACT_SIZE --assoc_act=$ASSOC_ACT --size_own=$SIZE_OWN --reset_tick=$RESET_TICK --cmd='$BENCHMARKS_BIN_DIR/$INPUT_BENCH' --options='$BENCH_OPTION'"
