...
// FalseSharing: gc_word_t size 8 byte alias for uintrptr_t
// 
...
// FalseSharing: falsely shared structure reported by Feather
...
void gc_init(gc_word_t (*epoch)())
{
  ...
  current_epoch = epoch;
  // FalseSharing: allocation of thread array falsely shared
  #ifdef FIX_FS
...
void gc_set_epoch(gc_word_t epoch)
{
  ...
  ATOMIC_STORE(&threads[idx].ts, epoch); // FalseSharing: line reported by Feather
}
