  ...
  while (AO_load_full(&stop) == 0) {
    ...
    if (unext) { // update
      ...
      if (last < 0) { // add
        ...
        d->nb_trans++;
        tloc[id * (64 / sizeof(ulong))]++; // FalseSHaring: reported for this line
        d->nb_remove++;
      }
...
int main(int argc, char** argv) {
  ...
  unsigned long reads, effreads, updates, effupds, aborts, aborts_locked_read, aborts_locked_write,
      aborts_validate_read, aborts_validate_write, aborts_validate_commit, aborts_invalid_memory,
      aborts_double_write, max_retries, failures_because_contention;
  thread_data_t* data;
    ...
    switch (c) {
      ...
      case 'h':
        printf(
            ...
            "  -f, --effective <int>\n"
            "        update txs must effectively write (0=trial, 1=effective, default=" XSTR(
                DEFAULT_EFFECTIVE) ")\n"
                                   "  -d, --duration <int>\n"
                                   "        Test duration in milliseconds (0=infinite, "
                                   "default=" XSTR(
                                       DEFAULT_DURATION) ")\n"
                                                         "  -i, --initial-size <int>\n"
                                                         "        Number of elements to insert "
                                                         "before test (default=" XSTR(
                                                             DEFAULT_INITIAL) ")\n"
                                                                              "  -t, --thread-num "
                                                                              "<int>\n"
                                                                              "        Number of "
                                                                              ...
                                                                              "(default=" XSTR(
                                                                                  DEFAULT_NB_THREADS) ")\n"
                                                                                                      "  -r, --range <int>\n"
                                                                                                      "        Range of integer values inserted in set (default=" XSTR(
                                                                                                          DEFAULT_RANGE) ")\n"
                                                                                                                         "  -S, --seed <int>\n"
                                                                                                                         "        RNG seed (0=time-based, default=" XSTR(
                                                                                                                             DEFAULT_SEED) ")\n"
                                                                                                                                           "  -u, --update-rate <int>\n"
  ...
  // FalseSharing: aligned these access allocations
  //   if ((data = (thread_data_t*)malloc(nb_threads * sizeof(thread_data_t))) == NULL) {
  int ret = posix_memalign((void **)&data, 64,  nb_threads * (sizeof(thread_data_t) + (64 - (sizeof(thread_data_t) % 64))));
  if (ret != 0) {
  ...
  if ((maintenance_data = (maintenance_thread_data_t*)malloc(
           nb_maintenance_threads * sizeof(maintenance_thread_data_t))) == NULL) {
    perror("malloc");
    exit(1);
  }
  if ((maintenance_threads = (pthread_t*)malloc(nb_maintenance_threads * sizeof(pthread_t))) ==
      NULL) {
    perror("malloc");
    exit(1);
  }
  ...
  printf("Set size     : %d\n", size);
  printf("Tree size     : %d\n", tree_size);
  printf("Size of thread_data_t :%ld\n", sizeof(thread_data_t));
  //printf("Level max    : %d\n", levelmax);
...
// m5_reset_stats(0,0);
  ...
  for (i = 0; i < nb_maintenance_threads; i++) {
    ...
    if ((maintenance_data[i].t_nb_trans =
             (unsigned long*)malloc(nb_threads * sizeof(unsigned long))) == NULL) {
      perror("malloc");
      exit(1);
    }
    ...
    if ((maintenance_data[i].t_nb_trans_old =
             (unsigned long*)malloc(nb_threads * sizeof(unsigned long))) == NULL) {
      perror("malloc");
      exit(1);
    }
    ...
    if (pthread_create(&maintenance_threads[i], &attr, test_maintenance,
                       (void*)(&maintenance_data[i])) != 0) {
      fprintf(stderr, "Error creating thread\n");
      exit(1);
    }
  }
