...
void avl_delete_node(avl_node_t* node) { avl_delete_node_free(node, 0); }
...
avl_intset_t* avl_set_new() { return avl_set_new_alloc(0, 0); }
...
avl_intset_t* avl_set_new_alloc(int transactional, long nb_threads) {
  ...
  // set->t_nbtrans = (ulong*)malloc(nb_threads * sizeof(ulong)); // old allocation
  int ret =
      posix_memalign((void**)&(set->t_nbtrans), 64, nb_threads * 64); // FalseSharing: allocation modified
  if (ret != 0){
    perror("posix_memalign");
    exit(1);
  }
  // set->t_nbtrans_old = (ulong*)malloc(nb_threads * sizeof(ulong));
  ret = posix_memalign((void **)&(set->t_nbtrans_old), 64, nb_threads * sizeof(ulong));
  if (ret != 0){
    perror("posix_memalign");
    exit(1);
  }
  // FalseSharing: leading to false sharing, size of ulong << 64
  // set->nb_committed = (ulong*)malloc(nb_threads * sizeof(ulong));
  ret = posix_memalign((void **)&(set->nb_committed) ,64, nb_threads * 64);
  if (ret != 0){
    perror("posix_memalign");
    exit(1);
  }
  // each thread access first byte of each block
  for (i = 0; i < nb_threads; i++) {
    set->nb_committed[i * (64 / sizeof(ulong))] = 0;
  }
  // set->nb_committed_old = (ulong*)malloc(nb_threads * sizeof(ulong));
  ret = posix_memalign((void**)&(set->nb_committed_old), 64, nb_threads * sizeof(ulong));
  if (ret != 0){
    perror("posix_memalign");
    exit(1);
  }
 ...
 * =============================================================================
 */
rbtree_t* rbtree_alloc(long (*compare)(const void*, const void*), long nb_threads) {
 ...
 * =============================================================================
 */
rbtree_t* TMrbtree_alloc(TM_ARGDECL long (*compare)(const void*, const void*)) {
