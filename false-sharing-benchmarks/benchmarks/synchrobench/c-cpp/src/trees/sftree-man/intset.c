...
inline int avl_req_seq_add(avl_node_t* parent, avl_node_t* node, val_t val, val_t key, int go_left,
                           int* success) {
  avl_node_t* new_node;
  int ret;
#ifdef SEPERATE_BALANCE2
...
int avl_seq_check_rotations(avl_node_t* parent, avl_node_t* node, int lefth, int righth,
                            int go_left) {
...
int avl_seq_req_find_successor(avl_node_t* parent, avl_node_t* node, int go_left,
                               avl_node_t** succs) {
  int ret;
...
int avl_delete(val_t key, const avl_intset_t* set) {
  ...
  if (ret == 1 && set->active_remove &&
      ((avl_node_t*)TX_UNIT_LOAD(&place->left) == NULL ||
       (avl_node_t*)TX_UNIT_LOAD(&place->right) == NULL)) {
#ifdef REMOVE_LATER
...
int avl_single_rotate(avl_node_t* parent, int go_left, avl_node_t* node, int left_rotate,
                      int right_rotate, avl_node_t** child_addr) {
      ...
      if (bal >= 2 || right_rotate) {
        ...
        if (child != NULL) {
          ...
          if (lefth - child_localh == 0) {
            ret = avl_right_rotate(parent, go_left, node, lefth, righth, child, child_addr,
                                   right_rotate);
          }
        }
        ...
        if (child != NULL) {
          ...
          if (righth - child_localh == 0) {
            ret = avl_left_rotate(parent, go_left, node, lefth, righth, child, child_addr,
                                  left_rotate);
          }
        }
      }
...
int avl_right_rotate(avl_node_t* parent, int go_left, avl_node_t* node, val_t lefth, val_t righth,
                     avl_node_t* left_child, avl_node_t** left_child_addr, int do_rotate) {
...
int avl_left_rotate(avl_node_t* parent, int go_left, avl_node_t* node, val_t lefth, val_t righth,
                    avl_node_t* right_child, avl_node_t** right_child_addr, int do_rotate) {
...
int recursive_node_propagate(avl_intset_t* set, balance_node_t* node, balance_node_t* parent,
                             free_list_item* free_list, uint depth, uint depth_del) {
  ...
  usleep((THROTTLE_TIME) /
         (set->deleted_count + (set->current_deleted_count * set->current_deleted_count) + 1));
#elif defined BUSY_WAITING
  sleepTime = (THROTTLE_TIME) /
              (set->deleted_count + (set->current_deleted_count * set->current_deleted_count) + 1);
  while (sleepTime > 0) {
    sleepTime--;
  }
...
int recursive_node_propagate(avl_intset_t* set, avl_node_t* node, avl_node_t* parent, uint depth,
                             uint depth_del) {
...
=============================================================================
  */
TM_CALLABLE
bool_t TMrbtree_contains(TM_ARGDECL rbtree_t* r, void* key) {
  int ret;
...
inline int avl_req_seq_update(avl_node_t* parent, avl_node_t* node, val_t val, val_t key,
                              int go_left, int* success) {
  avl_node_t* new_node;
  int ret;
...
void do_maintenance(avl_intset_t* tree) {
  ...
  for (i = 0; i < nb_threads; i++) {
    t_nb_trans[i * (64 / sizeof(ulong))] = nb_committed[i * (64 / sizeof(ulong))];
    if (t_nb_trans[i * (64 / sizeof(ulong))] == t_nb_trans_old[i]) {
      done = 1;
      break;
    }
  }
...
void check_maintenance(avl_intset_t* tree) {
  ...
  if (nextHelp == id) {
    // FalseSharing: array is aligned to 64 byte boundary
    if (tree->nb_committed[id * (64 / sizeof(ulong))] >=
        THROTTLE_UPDATE + tree->nb_committed_old[id]) {
      do_maintenance(tree);
      tree->nb_committed_old[id] = tree->nb_committed[id * (64 / sizeof(ulong))];
    }
