...
typedef struct barrier {
  pthread_cond_t complete; // 48 byte
  pthread_mutex_t mutex;   // 40 byte
  int count;               // 4 byte
  int crossing;            // 4 byte
} barrier_t;
...
// FalseSharing: 212 byte: ~4 cache block, may use padding to byte align
typedef struct thread_data {
  val_t first;               // FalseSharing: lazy-list/linkedlist-lock.h intptr_t 8byte
  long range;                // 8 byte, 16 byte
  int update;                // 4 byte, 20 byte
  int move;                  // 4 byte, 24 byte
  int snapshot;              // 4 byte, 28 byte
  int unit_tx;               // 4 byte, 32 byte
  int alternate;             // 4 byte, 36 byte
  int effective;             // 4 byte, 40 byte
  unsigned long nb_add;      // 8 byte, 48 byte
  unsigned long nb_added;    // 8 byte, 56  byte
  unsigned long nb_remove;   // 8 byte, 64 byte
  unsigned long nb_removed;  // 8 byte, 72 byte
  unsigned long nb_contains; // 8 byte, 80 byte
  /* added for HashTables */
  unsigned long load_factor;   // 8 byte, 88 byte
  unsigned long nb_move;       // 8 byte, 96 byte
  unsigned long nb_moved;      // 8 byte, 104 byte
  unsigned long nb_snapshot;   // 8 byte, 112  byte
  unsigned long nb_snapshoted; // 8 byte, 120 byte
  /* end: added for HashTables */
  unsigned long nb_found;                  // 128 byte
  unsigned long nb_aborts;                 // 136 byte
  unsigned long nb_aborts_locked_read;     // 144 byte
  unsigned long nb_aborts_locked_write;    // 152 byte
  unsigned long nb_aborts_validate_read;   // 160 byte
  unsigned long nb_aborts_validate_write;  // 168 byte
  unsigned long nb_aborts_validate_commit; // 176 byte
  unsigned long nb_aborts_invalid_memory;  // 184 byte
  unsigned long max_retries;               // 192 byte
  unsigned int seed;                       // 196 byte
  ht_intset_t* set;                        // 204 byte
  barrier_t* barrier;                      // 212 byte
  char padding[44];                        // 256 byte
} thread_data_t;
...
int main(int argc, char** argv) {
  ...
  ht_intset_t* set; // 8 byte
  int i, c, size;   // 4+4+4 byte
  val_t last = 0;   // 8byte
  val_t val = 0;    // 8byte
  unsigned long reads, effreads, updates, effupds, moves, moved, snapshots, snapshoted, aborts,
      aborts_locked_read, aborts_locked_write, aborts_validate_read, aborts_validate_write,
      aborts_validate_commit, aborts_invalid_memory, max_retries; // 16* 8 bytes
  thread_data_t* data;                                            // 8bytes
  pthread_t* threads;                                             // 8bytes
  pthread_attr_t attr;                                            // 56 bytes
  barrier_t barrier;                                              // 96 bytes
  struct timeval start, end;                                      // 32(16+16)+32
  struct timespec timeout;                                        // 16 bytes
  ...
  int effective = DEFAULT_EFFECTIVE; // 13*4 bytes=52 bytes
  sigset_t block_set;                // 128 bytes
                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                           "        Number of threads (default=" XSTR(DEFAULT_NB_THREADS) ")\n"
                                                                                                                                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                                                                                                                                          "        Range of integer values inserted in set (default=" XSTR(DEFAULT_RANGE) ")\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                                                                                                                                                                                                                          "        RNG seed (0=time-based, default=" XSTR(DEFAULT_SEED) ")\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        "  -u, --update-rate <int>\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        "        Percentage of update transactions (default=" XSTR(
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            DEFAULT_UPDATE) ")\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "  -a , --move-rate <int>\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "        Percentage of move transactions (default=" XSTR(
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                DEFAULT_MOVE) ")\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              "  -s , --snapshot-rate <int>\n"
  ...
  printf("Type sizes   : int=%d/long=%d/ptr=%d/word=%d\n", (int)sizeof(int), (int)sizeof(long),
         (int)sizeof(void*), (int)sizeof(uintptr_t));

  timeout.tv_sec = duration / 1000;
  timeout.tv_nsec = (duration % 1000) * 1000000;

  //   if ((data = (thread_data_t*)malloc(nb_threads * sizeof(thread_data_t))) == NULL) {
  if ((posix_memalign((void**)&data, 64, nb_threads * sizeof(thread_data_t))) != 0) {
  ...
  assert((tmp & 0x3F) == 0);
  /*
  int ret = posix_memalign((void**)&node_l, 64, sizeof(node_l_t));
  if (ret != 0) {
    perror("posix_memalign");
    exit(1);
  }
  */
  ...
  stop = 0; // lazy-list/linkedlist-lock.h
...
//FalseSharing: reset stats before launching threads
//Data init (serial) included in running may affect parallelism
// m5_reset_stats(0,0);
#ifdef ROI_TRACING
  roi_begin();
#endif
  ...
  if (duration > 0) {
    ...
    sigemptyset(&block_set); // intialize the signal
    sigsuspend(&block_set);  // suspend the main thread if time negative
  }
  AO_store_full(&stop, 1); //FalseSharing: generalize.h in src/atomic_ops.h
  ...
  for (i = 0; i < nb_threads; i++) {
    ...
    */
    ...
    effreads += data[i].nb_contains + (data[i].nb_add - data[i].nb_added) +
                (data[i].nb_remove - data[i].nb_removed) + (data[i].nb_move - data[i].nb_moved) +
                data[i].nb_snapshoted;
  ...
  printf("#txs          : %lu (%f / s)\n", reads + updates + moves + snapshots,
         (reads + updates + moves + snapshots) * 1000.0 / duration);
