...
void *test(void *data) {
  ...
  while (stop == 0) {
	...
	if (d->alternate) { // alternate mode
	  ...
	  val = rand_range_re(&d->seed, d->range); // FalseSharing: reported by perf
    ...
    if (d->effective) { // a failed remove/add is a read-only tx
      unext = ((100 * (d->nb_added + d->nb_removed)) // FalseSharing: reported  by perf
	       < (d->update * (d->nb_add + d->nb_remove + d->nb_contains))); // FalseSharing: reported by perf
