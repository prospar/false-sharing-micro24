...
int parse_insert(intset_l_t *set, val_t val) {
	...
	while (1) {
		...
		// FalseSharing: pthread mutex_lock call define in linkedlist-lock.h
		LOCK(&pred->lock);
		LOCK(&curr->lock);
...
int parse_delete(intset_l_t *set, val_t val) {
	node_l_t *pred, *curr; // FalseSharing: reported by Feather
	int result, validated, isVal;
	while(1) {
		pred = set->head;
		curr = get_unmarked_ref(pred->next);
		while (curr->val < val) { // FalseSharing: reported by perf
			pred = curr;
			curr = get_unmarked_ref(curr->next); // FalseSharing: reported by perf
		}
		// FalseSharing: pthread mutex_lock call define in linkedlist-lock.h
		LOCK(&pred->lock);
		LOCK(&curr->lock);
