...
// FalseSharing: Feather: listing 1
// val_t is intptr_t
// FalseSharing: separate only lock variable to a seperate cache line
typedef struct node_l {
  val_t val;              // 8 bytes
  struct node_l* next;    // 8 bytes
  uint64_t PAD2[6];       // 48 bytes
  volatile ptlock_t lock; // 40 bytes // on new cache line
  uint64_t PAD3[3];       // PADDING OF 24 bytes
} node_l_t;
