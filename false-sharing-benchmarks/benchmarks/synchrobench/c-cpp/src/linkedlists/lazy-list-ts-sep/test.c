    ...
    if (unext) { // update
      ...
      if (last < 0) { // add
        ...
        if (d->alternate) { // alternate mode
          ...
          val = rand_range_re(&d->seed, d->range);        // FalseSharing: Wr to val
          if (set_remove_l(d->set, val, TRANSACTIONAL)) { // FalseSharing: Rd to val
            d->nb_removed++;
            last = -1;
          }
        }
    ...
    switch (c) {
      ...
      case 'h':
        printf(
            ...
            "  -h, --help\n"
            "        Print this message\n"
            "  -A, --alternate (default=" XSTR(
                DEFAULT_ALTERNATE) ")\n"
                                   "        Consecutive insert/remove target the same value\n"
                                   "  -f, --effective <int>\n"
                                   "        update txs must effectively write (0=trial, "
                                   "1=effective, default=" XSTR(
                                       DEFAULT_EFFECTIVE) ")\n"
                                                          "  -d, --duration <int>\n"
                                                          "        Test duration in milliseconds "
                                                          "(0=infinite, default=" XSTR(
                                                              DEFAULT_DURATION) ")\n"
                                                                                "  -i, "
                                                                                "--initial-size "
                                                                                "<int>\n"
                                                                                ...
                                                                                "(default=" XSTR(
                                                                                    DEFAULT_INITIAL) ")\n"
                                                                                                     "  -t, --thread-num <int>\n"
                                                                                                     "        Number of threads (default=" XSTR(
                                                                                                         DEFAULT_NB_THREADS) ")\n"
                                                                                                                             "  -r, --range <int>\n"
...
// m5_reset_stats(0,0);
  ...
  max_retries = 0;
  for (i = 0; i < nb_threads; i++) {
    /* FalseSharing: disabling printing per thread stats
    printf("Thread %d\n", i);
    printf("  #add        : %lu\n", data[i].nb_add);
    printf("    #added    : %lu\n", data[i].nb_added);
    printf("  #remove     : %lu\n", data[i].nb_remove);
