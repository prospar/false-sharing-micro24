...
uint32_t loop_count __attribute__((aligned(64)));
uint8_t array_with_fs[NUM_THREADS] __attribute__((aligned(64)));
...
int main(int argc, char* argv[]) {
  ...
  // Correctness check
  for (int i = 0; i < NUM_THREADS; i++) {
    // FalseSharing: always storing 1 in the index to support larger loop count
    if (array_with_fs[i] != 1) 
