 ...
 * The amount of true sharing >>> false sharing
 */
...
void* proportioanl_ts(void* threadId) {
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    pthread_create(&threads[i], NULL, &proportioanl_ts, (void*)(intptr_t)i);
  }
  ...
  int total_updates = 1;
  if (loop_count >> 13)
    total_updates = (loop_count >> 13);
  for (int i = 0; i < NUM_THREADS; i++) {
    if (fs_ts_struct.shared_arr[i] != total_updates) {
      printf("Diff found for index: %d actual value:%d\n", i, fs_ts_struct.shared_arr[i]);
    }
  }
  if (fs_ts_struct.shared_var != (loop_count << 2))
    printf("Diff found for shared_var: %d", fs_ts_struct.shared_var);
